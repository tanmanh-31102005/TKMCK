...
from configure_mpls_ldp import configure_mpls_ldp, verify_ldp

# Cờ chọn chế độ MPLS:
# True  = Dùng FRRouting (OSPF + LDP) theo file configure_mpls_ldp.py
# False = Dùng Static MPLS theo file configure_mpls.py
USE_DYNAMIC_LDP = True
...
def configure_full_network(net):
    ...
    # ---------- CE backbone routers ----------
    # Dùng 'replace' thay 'add' để tránh lỗi duplicate khi chạy lại
    # CE1: LAN1 via ce1_lan; các LAN khác via PE1
    net['ce1'].cmd('ip route replace 10.1.0.0/24  via 10.100.1.2 2>/dev/null')
    net['ce1'].cmd('ip route replace 10.2.0.0/16  via 10.0.1.2   2>/dev/null')
    net['ce1'].cmd('ip route replace 10.3.0.0/16  via 10.0.1.2   2>/dev/null')
    net['ce1'].cmd('ip route replace 10.100.1.0/30 dev ce1-eth1  2>/dev/null')

    # CE2: LAN2 via ce2_lan; các LAN khác via PE2
    net['ce2'].cmd('ip route replace 10.2.0.0/16  via 10.100.2.2 2>/dev/null')
    net['ce2'].cmd('ip route replace 10.1.0.0/24  via 10.0.2.2   2>/dev/null')
    net['ce2'].cmd('ip route replace 10.3.0.0/16  via 10.0.2.2   2>/dev/null')

    # CE3: LAN3 via ce3_lan; các LAN khác via PE3
    net['ce3'].cmd('ip route replace 10.3.0.0/16  via 10.100.3.2 2>/dev/null')
    net['ce3'].cmd('ip route replace 10.1.0.0/24  via 10.0.3.2   2>/dev/null')
    net['ce3'].cmd('ip route replace 10.2.0.0/16  via 10.0.3.2   2>/dev/null')

    # ---------- PE routers ----------
    # Khi LDP động: OSPF sẽ tự học prefix và LDP phân phối nhãn.
    # Chỉ cần cài static route về phía LAN của chính CE mình quản lý.
    # PE1
    net['pe1'].cmd('ip route replace 10.1.0.0/24   via 10.0.1.1 2>/dev/null')
    net['pe1'].cmd('ip route replace 10.100.1.0/30 via 10.0.1.1 2>/dev/null')
    if not USE_DYNAMIC_LDP:
        net['pe1'].cmd('ip route replace 10.2.0.0/16  via 10.10.11.2 2>/dev/null')  # P1→P3→PE2
        net['pe1'].cmd('ip route replace 10.3.0.0/16  via 10.10.11.2 2>/dev/null')  # P1→P2→PE3

    # PE2
    net['pe2'].cmd('ip route replace 10.2.0.0/16   via 10.0.2.1 2>/dev/null')
    net['pe2'].cmd('ip route replace 10.100.2.0/30 via 10.0.2.1 2>/dev/null')
    if not USE_DYNAMIC_LDP:
        net['pe2'].cmd('ip route replace 10.1.0.0/24  via 10.10.23.2 2>/dev/null')  # P3→P1→PE1
        net['pe2'].cmd('ip route replace 10.3.0.0/16  via 10.10.24.2 2>/dev/null')  # P4→PE3

    # PE3
    net['pe3'].cmd('ip route replace 10.3.0.0/16   via 10.0.3.1 2>/dev/null')
    net['pe3'].cmd('ip route replace 10.100.3.0/30 via 10.0.3.1 2>/dev/null')
    if not USE_DYNAMIC_LDP:
        net['pe3'].cmd('ip route replace 10.1.0.0/24  via 10.10.32.2 2>/dev/null')  # P2→P1→PE1
        net['pe3'].cmd('ip route replace 10.2.0.0/16  via 10.10.34.2 2>/dev/null')  # P4→PE2
