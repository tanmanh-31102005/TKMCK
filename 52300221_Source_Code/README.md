# Thiết Kế & Triển Khai Mạng Metro Ethernet sử dụng MPLS (Mã Nguồn)
...
* **Cách 1: Chạy tự động toàn bộ kịch bản kiểm thử (Khuyến nghị)**
  ...
  ```
  *(Các biểu đồ kết quả sẽ được lưu tại thư mục `results/charts/`)*
...
* **Cách 3: Chạy độc lập từng chi nhánh**
  ```bash
  sudo bash scripts/run_flat_tests.sh      # Chi nhánh 1
  sudo bash scripts/run_3tier_tests.sh     # Chi nhánh 2
  sudo bash scripts/run_leafspine_tests.sh # Chi nhánh 3
  ```
